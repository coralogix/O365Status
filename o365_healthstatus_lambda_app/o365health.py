...
class O365Health:
    ...
    def main (self, event, context):
        payload = 'client_id='+self.CLIENT_ID+'&scope=https%3A%2F%2Fgraph.microsoft.com%2F.default&client_secret='+self.CLIENT_SECRET+'&grant_type=client_credentials'
        rqst = requests.post('https://login.microsoftonline.com/'+self.TENANT+'/oauth2/v2.0/token', data = payload)
